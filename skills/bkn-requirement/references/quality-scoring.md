...
- `bkn_creator_handoff` 未先按场景做五层收敛，或未区分“业务已确认内容”“仍属建模候选”“需下游建模阶段判定的问题”；
- `bkn_creator_handoff` 不是中文业务可读摘要；
- P0/P1 场景正文未先完成业务化场景收敛，只在文末 handoff 中补对象 / 关系 / 动作清单；
- 场景收敛或五层交接摘要只列名词，没有说明为何该场景需要这些对象、联系、业务逻辑或治理控制；
...
- 场景收敛或交接摘要无法用业务语言追溯来源场景、证据来源或确认状态；
...
- P0/P1 场景缺少本体分类结论，导致下游无法判断哪些是对象、属性、枚举、关系、结果、指标、算子、逻辑能力或行动候选；
...
| 41-60 | 有场景级摘要，但业务收敛不完整或“业务已确认内容”混入待确认内容。 |
| 61-80 | P0/P1 场景正文已完成业务化四层收敛，并有中文业务可读五层交接摘要；仍有少量边界问题。 |
| 81-100 | 每个 P0/P1 场景都能从业务目标、规则和验收自然推导出正文四层内容和第 15 节五层交接摘要，且有中文证据、确认状态和明确 BKN_Creator 路由。 |
