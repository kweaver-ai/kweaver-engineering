...
`bkn_creator_handoff` 必须分三层，并始终使用业务语言：

| 层级 | 内容 |
|---|---|
| 业务已确认内容 | 业务专家可直接确认的场景、业务对象、规则、系统和验收用例。 |
| 仍属建模候选 | AI 工程师根据 PRD 推导出的候选对象、关系、逻辑属性、Action、Skill、数据视图。 |
| 需下游建模阶段判定的问题 | 需要下游决定的建模问题，如对象边界、关系对象、逻辑属性挂载、Action 执行模式。 |
